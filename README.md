...
This proof of concept is a **Python package + a showcase notebook**, not a web product.
...
```powershell
jupyter notebook gadded.ipynb          # runs the full pipeline on the golden case
...
```
src/gadded/        analytical modules (contracts, weather, pv, load, matching,
                   optimization, finance, risk, gis, regulatory, vendors,
                   feasibility, report)
data/              golden_case.json, assumptions.json, cached weather,
                   load archetypes, regulatory excerpts, zones.geojson
gadded.ipynb       showcase: input -> full assessment
